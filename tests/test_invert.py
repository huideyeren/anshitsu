...
def test_invert_by_rgb(setup):
    image = Image.open(
        os.path.join(".", "tests", "pic", "dog.jpg"),
    )
    rt = retouch.Retouch(image=image, invert=True)
    gray = rt.process()
    assert gray.mode == "RGB"


def test_invert_by_grayscale(setup):
    image = Image.open(
...
def test_invert_by_grayscale_with_alpha(setup):
    image = Image.open(
